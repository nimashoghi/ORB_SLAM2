...
std::unique_ptr<ORBVocabulary> System::parse_vocabulary(const std::string &vocabulary_file)
{
    ...
    auto vocabulary = make_unique<ORBVocabulary>();
    if (!vocabulary->loadFromTextFile(vocabulary_file))
    {
        cerr << "Wrong path to vocabulary. " << endl;
        cerr << "Falied to open at: " << vocabulary_file << endl;
        exit(-1);
    }
...
System::System(ORBVocabulary *vocabulary, const string &strSettingsFile, const eSensor sensor,
               const bool bUseViewer) : mSensor(sensor), mpVocabulary(vocabulary), mpViewer(static_cast<Viewer *>(NULL)), mbReset(false), mbActivateLocalizationMode(false),
...
System::eSensor System::Sensor() const
{
    return mSensor;
}
